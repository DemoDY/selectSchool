package com.select.school.dao.impl;
...
import com.select.school.dao.IllegalClassTypeException;
...
public class QueryDaoImpl extends SqlSessionDaoSupport implements QueryDao {
     ...
     * @param： pageControllerInfo 分页条件和排序条件，如果为空或者分页条件不足，则默认使用不分页的查询
     * @author: gc
     * @创建时间: 2017-05-20 11:13:23
     */
    @SuppressWarnings({ "unchecked", "rawtypes" })
    public <E> List<E> executeForObjectList(String sqlID, Object bindParams, PageControllerInfo pageControllerInfo) {
        if (log.isDebugEnabled()) {
            log.debug("executeForObjectList Start.");
        }
        log.info(MessageFormat.format("sqlId[{0}]，输入参数：[{1}]", sqlID, bindParams));
        ...
        SqlSession sqlMapTemp = this.getSqlSession();
        if(pageControllerInfo != null) {
            if(pageControllerInfo.getOrderBy() != null && !"".equals(pageControllerInfo.getOrderBy())) {
                PageHelper.orderBy(pageControllerInfo.getOrderBy());
            }
        }
        List<E> list = sqlMapTemp.selectList(sqlID, bindParams);
        if (log.isDebugEnabled()) {
    ...
    @SuppressWarnings({ "unchecked", "rawtypes" })
    public List<Map<String, Object>> executeForMapList(String sqlID, Object bindParams, PageControllerInfo pageControllerInfo) throws Exception {
        if (log.isDebugEnabled()) {
            log.debug("executeForMapList Start.");
        }
        ...
        if(pageControllerInfo == null 
                || pageControllerInfo.getPageSize() == null
                || pageControllerInfo.getPageNum() == null) {
            return this.executeForObjectList(sqlID, bindParams, pageControllerInfo);
        }
        //2017-12-6 10:49:43-刘运兵-修改：为map分页的方法添加排序功能，之前的仅仅能分页不能排序
        //修改参数列表  pageControllerInfo.getPageNum(), pageControllerInfo.getPageSize() 变为：PageControllerInfo pageControllerInfo
        List mapList = this.executeForMapListByPage(sqlID, bindParams, pageControllerInfo);
        if (log.isDebugEnabled()) {
            log.debug("executeForMapList End.");
        }
     ...
     */
    @SuppressWarnings({ "unchecked", "rawtypes" })
    public <E> List<E> executeForObjectListByPage(String sqlID, Object bindParams, PageControllerInfo pageControllerInfo) throws Exception {
        if (log.isDebugEnabled()) {
            log.debug("executeForObjectListByPage Start.");
        }
        ...
        if(pageControllerInfo == null 
                || pageControllerInfo.getPageSize() == null
                || pageControllerInfo.getPageNum() == null) {
            return executeForObjectList(sqlID, bindParams, pageControllerInfo);
        }
        ...
        stopWatch.start();
        SqlSession sqlMapTemp = this.getSqlSession();
        Integer pageSize = pageControllerInfo.getPageSize();
        Integer pageNum = pageControllerInfo.getPageNum();
        ...
        PageHelper.startPage(pageNum, pageSize);
        String orderBy = pageControllerInfo.getOrderBy();
        if(orderBy != null && !"".equals(orderBy)) {
            PageHelper.orderBy(orderBy);
        }
        List list = sqlMapTemp.selectList(sqlID, bindParams);
    ...
    @SuppressWarnings({ "unchecked", "rawtypes" })
    public List<Map<String, Object>> executeForMapListByPage(String sqlID, Object bindParams, PageControllerInfo pageControllerInfo) throws Exception {
        if (log.isDebugEnabled()) {
        ...
        List mapList = this.executeForObjectList(sqlID, bindParams, pageControllerInfo);
        if (log.isDebugEnabled()) {
